...
def commit(message):
    ...
    files.update_red('HEAD', files.RefValue(symbolic=False, value=obj_id))
    return obj_id
...
def merge(other):
    ...
    if merge_base == HEAD:
        ...
        files.update_ref('HEAD',
                         files.RefValue(symbolic=False, value=other))
...
_commit = namedtuple('Commit', ['tree', 'parent', 'message'])
...
def get_commit(obj_id):
    ...
    message = '\n'.join(lines)
    return _commit(tree=tree, parent=parent, message=message)
